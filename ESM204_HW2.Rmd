...
```
  ...
  **2) calculate the total cost of meeting the target using that approach**
  ...
  The total cost of the policy is the T* x water use, plug the cost of reduction which is the area under the aggregate line up to the reduction amount. 
  
  This is (158.355*500) + (255 * 158.355 * .5) = $99,367.76

```{r, results='hide'}
(255*158.355*.5) + (158.355*500)
```
...
mojave_cost <- calc_cost(mod_mojave, quantity_abate = 47.43) 
antelope_cost <- calc_cost(mod_antelope, quantity_abate = 74.36) 
ventura_cost <- calc_cost(mod_ventura, quantity_abate = 82.81) 
#these calculations are the same thing as if we calculated the area under the curve using the area of a triangle

district_costs <- tibble(kern_cost, mojave_cost, antelope_cost, ventura_cost) 
#put costs into nice table
district_costs %>% 
  kableExtra::kable(col.names = c("Kern",
                                  "Mojave", "Antelope", "Ventura")) %>% 
   kable_styling(full_width = FALSE)
...
```
  ...
  **2) calculate the total cost of meeting the target using that approach**
  ...
  This is 455 * 158.355 * .05 = $3602.58
